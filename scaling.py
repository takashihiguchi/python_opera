...
# flist = glob("superB*_000A")
# ident = 'typeB_'

flist = glob(str(sys.argv[1]))
...
for i in range(3):
    for j in range(2):
        ...
        axY[j,i].set_ylabel('$B_y$ (G)')
        axY[j,i].set_ylim(Bmin,Bmax)
...
for i in range(3):
    for j in range(2):
        ...
        axZ[j,i].set_ylabel('$B_z$ (G)')
        axZ[j,i].set_ylim(Bmin,Bmax)
